/*
 Copyright (C) 2015 - 2018, 2020 - 2022, 2024 - 2026 3NSoft Inc.
...
import { DATA_DIR_NAME, DEFAULT_SIGNUP_URL, isInAsar, toAsarUnpacked, PLATFORM_NAME } from './bundle-confs';
...
export const appDir = (() => {
	...
	if (process.platform === 'win32') {
		if (process.env.PORTABLE_EXECUTABLE_DIR) {
			return join(process.env.PORTABLE_EXECUTABLE_DIR, `${DATA_DIR_NAME}.AppImage.home`, DATA_DIR_NAME);
		}
		const parentFolder = (
			process.env.LOCALAPPDATA ??
			homedir()
		);
		return join(parentFolder, DATA_DIR_NAME);
	} else {
...
export async function denoBinParams(): Promise<DenoParams|undefined> {
	...
	if (!denoParamsDiscoveryProc) {
		denoParamsDiscoveryProc = (CUSTOM_DENO_RUNTIME ?
			testDenoBin(CUSTOM_DENO_RUNTIME, false) :
			testDenoBin(PACKED_DENO_BIN, false)
		)
		.catch(() => null)
		.then(params => {
			denoParams = params;
