...
export class InitProc {
	...
	private readonly mounting: { makeUserMount: MakeUserMount; unmount: () => Promise<void>; }|undefined;
	...
	constructor(
		...
		const mounting = mountIntoOS();
		if (testStandConf) {
			const { conf, filePath } = testStandConf;
			this.testStand = new TestStand(conf, filePath, this.exit.bind(this), mounting?.makeTestStandMount);
		} else {
			this.testStand = undefined;
		}
		if (mounting) {
			const { makeUserMount, unmount } = mounting;
			this.mounting = { makeUserMount, unmount };
		} else {
	...
	private startUserInTest(userId: string): AppsRunnerForTesting {
		...
		return {
			...
			// XXX we'll uncomment impl method, but while it is commented out, we pass a noop
			// openSite: (() => {}) as any, // apps.openSite.bind(apps),
			openSite: apps.openSite.bind(apps),
			listInstalled: apps.listInstalled.bind(apps),
			getAppStorage: apps.getAppStorage.bind(apps)
		};
	}
