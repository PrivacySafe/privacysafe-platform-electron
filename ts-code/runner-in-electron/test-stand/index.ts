...
import { checkAndTransformAddress, toCanonicalAddress, areAddressesEqual } from "../../platform/lib-common/canonical-address";
import { APP_ROOT_FOLDER, MANIFEST_FILE, SITE_ROOT_FOLDER } from "../caps/system/system-places/unpack-zipped-app";
import { dirname, isAbsolute, join, resolve } from "path";
import { readFileSync, statSync, writeFileSync } from "fs";
import { errWithCause } from "../../platform/lib-common/exceptions/error";
import type { MakeTestStandMount } from "../../platform/core";
import { Code } from "../../platform/lib-common/exceptions/file";
import { stringOfB64CharsSync, stringOfB64UrlSafeCharsSync } from "../lib-common/random-node";
import { MAIN_GUI_ENTRYPOINT } from "../../platform/lib-common/manifest-utils";
import { MapOfSets } from "../../platform/lib-common/map-of-sets";
...
export class TestStand {
	...
	constructor(
		...
		private readonly mounter: MakeTestStandMount|undefined
	) {
	...
	private async startForUsers(
		...
		const mounts = this.mounter?.();
		for (const { userParams, runner } of userParamsAndRunners) {
			await this.startDevAppsForUser(userParams, runner, mounts);
		}
		for (const { userParams, runner } of userParamsAndRunners) {
	...
	private async startDevAppsForUser(
		userParams: DevUserParams, runner: AppsRunnerForTesting, mounts: Promise<Mounter>|undefined
	): Promise<void> {
		...
		if (mounts) {
			mounts.then(async mounts => {
				...
				for (const appDomain of this.devApps.keys()) {
					const storages = runner.getAppStorage(appDomain);
					mounts.mountFS([userId, appDomain, 'local'], await storages('local'));
					mounts.mountFS([userId, appDomain, 'synced'], await storages('synced'));
				}
			}).catch(err => {
				console.error(`Fail to mount test apps' folders:`, err);
			})
		}
