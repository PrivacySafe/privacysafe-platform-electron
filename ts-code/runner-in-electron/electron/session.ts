...
import { desktopCapturer, session, Session, MediaAccessPermissionRequest, DisplayMediaRequestHandlerHandlerRequest, DesktopCapturerSource, SourcesOptions, NativeImage, systemPreferences, WebRequestFilter } from 'electron';
...
import { ensureDeviceAllowsScreenCapture } from '../caps/media-devices/media-devices';
import { AppFolder } from '../../platform/apps/app';

type ReadonlyFS = web3n.files.ReadonlyFS;
type RequestedCAPs = web3n.caps.RequestedCAPs;
type URLWhitelistEntry = web3n.caps.URLWhitelistEntry;
...
export async function makeSessionForApp(
	appDomain: string, appFilesRoot: AppFolder, w3nSetup: W3NSetupType,
	...
	const appBaseUrl = `${protoSchemas.W3N_APP.scheme}://${appDomain}`

	const allowAppResource = makeUrlChecker(appBaseUrl, devTools);

	const allowFetch = makeFetchUrlChecker(capsReq?.connectToExternal?.fetch);
	if (allowFetch) {
	...
	appSes.webRequest.onBeforeRequest((details, cb) => {
		if (allowAppResource(details.url) || allowFetch?.(details.url)) {
			cb({ cancel: false });
		} else {
...
function isWhitelistEntryOK({ schema, domain, pathPrefix }: URLWhitelistEntry): boolean {
	return (
		((schema === 'https') || (schema === 'ws')) &&
		(typeof domain === 'string') &&
		((pathPrefix === undefined) || (typeof pathPrefix === 'string'))
	);
}

function makeFetchUrlChecker(
	whitelist: URLWhitelistEntry[]|undefined
...
function filterFrom(whitelist: URLWhitelistEntry[]): WebRequestFilter {
	const urls = whitelist
	.filter(isWhitelistEntryOK)
	.map(({ schema, domain }) => `${schema}://${domain}/*`);
	return { urls };
}
